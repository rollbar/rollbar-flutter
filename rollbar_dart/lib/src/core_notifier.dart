...
class CoreNotifier {
  ...
  Future<void> log(
    ...
    var data = Data()
      ...
      ..client = Client.fromPlatform(rootPackage: _config.package)
      ..environment = _config.environment
      ..notifier = {'version': notifierVersion, 'name': notifierName}
      // Root detection compatibility, currently checked under 'server'
      ..server = {'root': _config.package};

    if (_transformer != null) {
      data = await _transformer!.transform(error, stackTrace, data);
    }

    final payload = Payload(accessToken: _config.accessToken, data: data);
